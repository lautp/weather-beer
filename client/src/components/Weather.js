		...
		<>
							...
							if (idx < 7) {
								return (
									<div key={idx} className="row">
										<div className="col-6 col-sm-4 col-md-4 h6">
											{weekday[date.getDay() + idx]}
										</div>
										<div
											className="col-1 col-sm-2 h6 d-flex justify-content-end"
											id={idx}>
											{weather !== null &&
												`${Math.round(weather[idx].temp.day)}`}
										</div>
										<div className="col-3 col-sm-2 h6">C°</div>
										<div className="col">
											<button
												className="btn btn-sm btn-outline-warning col-md-12 col-12 mb-3 mb-sm-3 mb-md-3"
												onClick={handleShow}>
												Order
											</button>
										</div>
			...
			<Modal
				...
				<Modal.Body>
					...
					<p>
						We recommend ordering {tmpt > 32 && 'Golden and NeIPA'}
						{tmpt > 25 && tmpt < 32 && 'Scotch and NeIPA'}
						{tmpt < 25 && 'Dubbel, Scotch and NeIPA'}
					</p>
				...
				<Modal.Footer>
					<Button variant="warning" onClick={addOrder}>
						Add to order
					</Button>
